    ...
    <div className="min-h-screen bg-[#f5f6f8] text-[#1a1a1a] font-sans antialiased">
      ...
      <main className="max-w-4xl mx-auto p-6">
        ...
        <div className="flex justify-between items-center mb-4">
          <p className="text-sm text-gray-500 font-medium tracking-tight">Showing <b>{filteredJobs.length}</b> applications</p>
        ...
        {/* Filter Panel (Full Restoration) */}
        {showFilters && (
          <div className="bg-white border border-gray-200 p-5 rounded-xl mb-6 shadow-sm animate-in slide-in-from-top-2 duration-200">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              <FilterGroup label="Search">
                <input type="text" placeholder="Company..." className="input-field" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
              </FilterGroup>
            ...
            <button onClick={() => { setSearchTerm(''); setFilterResume('all'); setFilterType('all'); setFilterStatus('all'); setFilterDate(''); }} className="mt-4 text-xs text-blue-600 font-bold underline">Reset All</button>
          </div>
        )}
            ...
            filteredJobs.map((job, i) => (
              <div key={i} className="bg-white border border-gray-200 p-4 rounded-xl shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 group hover:border-blue-200 transition-all">
                ...
                <div className="flex gap-2 border-t md:border-t-0 pt-3 md:pt-0">
                  {job.url && <a href={job.url} target="_blank" className="w-10 h-10 flex items-center justify-center bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-600 hover:text-white transition-all">↗</a>}
                  <button onClick={() => handleOpenModal(job)} className="px-5 py-2 border border-gray-200 text-xs font-bold rounded-lg hover:bg-gray-50 transition-all">Edit</button>
                </div>
              </div>
            ))
          }
